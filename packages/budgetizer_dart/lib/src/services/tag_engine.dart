...
class TagEngine {
  ...
  static TagEngine fromJson(String jsonStr) {
    ...
    final List<dynamic> tagList = data['tags'] as List<dynamic>;
    final tags =
        tagList.map((t) => Tag.fromJson(t as Map<String, dynamic>)).toList();
  ...
  Map<String, dynamic> toJson() {
    // Combine base tags and account tags into a single list for persistence
    final uniqueTags = <String, Tag>{};
    for (var tag in _tags) uniqueTags[tag.name] = tag;
    for (var tag in _accountTags) uniqueTags[tag.name] = tag;

    return {
      'tags': uniqueTags.values.map((t) => t.toJson()).toList(),
    };
  }
}

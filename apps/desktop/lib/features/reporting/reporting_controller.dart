...
class ReportingController extends AsyncNotifier<ReportingState> {
  ...
  Future<ReportingState> build() async {
    ...
    if (db.db != null) {
      try {
        final persistedNames = await db.getAllReportNames();
    ...
    if (!state.hasValue) {
      final lastViewed = await db.getSetting('last_viewed_report');
      if (lastViewed != null &&
    ...
    if (currentReport == 'All Budgets' || currentReport == 'budget') {
      ...
      // Load from DB
      try {
        final data = await db.getReport(currentReport);
        if (data != null) {
          // Parse report items
  ...
  Future<bool> saveReport(String reportName) async {
    ...
    final db = DatabaseService();
    try {
      await db.saveReport(reportName, data);
    } catch (e) {
      return false;
    }
    ...
    // Save as last viewed
    await db.saveSetting('last_viewed_report', reportName);
  ...
  Future<void> deleteReport(String reportName) async {
    final db = DatabaseService();
    try {
      await db.deleteReport(reportName);
    } catch (e) {
      return;
    }
    ...
    if (currentState.currentReportName == reportName) {
      ...
      await db.saveSetting('last_viewed_report', 'All Budgets');
      state = AsyncData(
        currentState.copyWith(
